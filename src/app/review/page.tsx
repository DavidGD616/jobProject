          ...
          <ol className="divide-y divide-[var(--ledger-border)]">
            {matches.map((match, index) => (
              <li className="review-card grid gap-5 px-6 py-7 sm:px-10 lg:grid-cols-[3.5rem_minmax(0,1fr)_14rem]" key={match.job.id}>
                ...
                <div className="flex flex-wrap content-start gap-2 lg:justify-end"><Link className="ledger-action" href={`/jobs/${match.job.id}`}>{match.triageDecision === "interested" ? "Open draft" : "View details"}</Link><form action={triageAction} className="flex flex-wrap gap-2"><input name="job_id" type="hidden" value={match.job.id} /><input name="profile_id" type="hidden" value={profile.id} /><input name="company_id" type="hidden" value={match.company.id} />{match.triageDecision !== "interested" ? <button className="ledger-action ledger-action-positive" name="decision" type="submit" value="interested">Interested</button> : null}<button className="ledger-action" name="decision" type="submit" value="skip">Skip</button><button className="ledger-action ledger-action-danger" name="decision" type="submit" value="block_company">Block company</button></form></div>
              </li>
            ))}
          </ol>
        ) : (
