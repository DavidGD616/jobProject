...
**`resume_json` is structured, not a blob.** This single choice is what makes Phase 4 possible — tailoring reorders source bullets while PDF rendering is a pure function of the structure. A stored PDF supports neither. Work-history entries carry the canonical employer title, dates, and bullets; every tailored variant retains them all. The optional `projects[].featured` flag and `projects[].completedAt` value are user-authored presentation metadata: featured projects are included first, and neither feature status nor chronology is inferred from array order ([ADR-0013](adr/0013-complete-work-history-tailoring.md)).
